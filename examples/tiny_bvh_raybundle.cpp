// Object viewer application, modified to use ray bundles of 64 rays.
// Ray bundles have a fixed size of TINYBVH_BUNDLE_RAYS rays (i.e., 64).
// A ray bundle can be traced through a BLAS using BVH4_CPU::IntersectBundle.
// A ray bundle can traverse a TLAS using BVH::IntersectBundle.
...
BVH4_CPU bvh;							// BVH layout to use. Must be BVH4_CPU for ray bundles.
...
void TraceWorkerThread( uint32_t* buf, int threadIdx )
{
	...
	Ray batch[TINYBVH_BUNDLE_RAYS]; // BVH4_CPU::IntersectBundle takes an array of 64 rays.
	int pidx[TINYBVH_BUNDLE_RAYS];
	for (int tiles = xtiles * ytiles, tile = threadIdx; tile < tiles; tile = tileIdx++)
	{
		const int tx = tile % xtiles, ty = tile / xtiles;
		for (int rayIdx = 0, y = 0; y < TILESIZE; y++) for (int x = 0; x < TILESIZE; x++)
		{
			...
			// add to batch
			pidx[rayIdx] = pixelIdx, batch[rayIdx++] = Ray( eye, D );
			if (rayIdx == TINYBVH_BUNDLE_RAYS)
			{
				// batch is full; trace rays.
				bvh.IntersectBundle( batch );
				// process intersection results.
				for( int i = 0; i < TINYBVH_BUNDLE_RAYS; i++ )
				{
					bvhvec3 E( 0.6f, 0.7f, 1 ); // sky color
					if (batch[i].hit.t < BVH_FAR)
					{
						bvhvec3 N = TriangleNormal( batch[i].hit.prim & PRIM_IDX_MASK );
						if (tinybvh_dot( N, batch[i].D ) > 0) N *= -1.0f;
						E = (N + 1) * 0.5f; // encode normal as rgb color
					}
					float r = 255.0f * E.x, g = 255.0f * E.y, b = 255.0f * E.z;
					// store color in Fenster pixel buffer
					buf[pidx[i]] = (int)b + ((int)g << 8) + ((int)r << 16);
				}
				// restart packet construction.
				rayIdx = 0;
			}
		}
	}
